...
// Produces characters from 'A' to 'Z' and loops back to 'A'
void Producer(void) {
    /*
        * @@@ [2 pt]
    ...
    while (1) {
        ...
        */
        if (Buffer_full == 1) { // consumer no consume
            ThreadYield();
...
void Consumer(void) {
    ...
    while (1) {
        ...
        * then clear the flag
        */
        if (Buffer_full == 0) { // producer no produce
            ThreadYield();
        } else { // producer produce
...
// Initializes global variables and sets up producer and consumer threads
void main(void) {
    /* 
