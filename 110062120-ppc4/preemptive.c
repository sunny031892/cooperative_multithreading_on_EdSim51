...
__data __at (0x33) ThreadID Next_ID; // Keep track of the next thread to run
...
void Bootstrap(void) {
    ...
    */
    ...
    // Set Next_ID to '1' initially
    // After the main thread (thread 0),
    // the next thread to run will be thread 1.
    Next_ID = '1';
...
void myTimer0Handler(void) {
    ...
    if (Cur_ID != '0') {
        ...
        // If the current thread is not thread 0
        // switch to thread 0 for execution next
        Cur_ID = '0';
    } else {
        // from consumer to producer, need to decide
        // If the current thread is thread 0
        // decide the next thread based on Next_ID
        Cur_ID = Next_ID;
        // Toggle Next_ID for the next interrupt
